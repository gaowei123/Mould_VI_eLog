...
{
	/// <summary>
	/// MouldingViDefectTracking_BLL
	/// </summary>
	public class MouldingViDefectTracking_BLL
	{
		private readonly Common.DAL.MouldingViDefectTracking_DAL dal=new Common.DAL.MouldingViDefectTracking_DAL();
		public MouldingViDefectTracking_BLL()
		{}
		#region  Method
        ...
        {
            return dal.UpdateCommandbypartNubmer(model,sPartNumber);
        }

        public SqlCommand UpdatebyDayMachineShifPartNumberAll(Common.Model.MouldingViTracking_Model model)
        {
            return dal.UpdatebyDayMachineShifPartNumberAll(model);
        }
        /// <summary>
        /// 删除一条数据
        /// </summary>
		...
		/// 获得数据列表
		/// </summary>
		public List<Common.Model.MouldingViDefectTracking_Model> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<Common.Model.MouldingViDefectTracking_Model> DataTableToList(DataTable dt)
		{
			List<Common.Model.MouldingViDefectTracking_Model> modelList = new List<Common.Model.MouldingViDefectTracking_Model>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				Common.Model.MouldingViDefectTracking_Model model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new Common.Model.MouldingViDefectTracking_Model();
		...
		/// 获得前几行数据
		/// </summary>
		public Common.Model.MouldingViDefectTracking_Model CopyObj(Common.Model.MouldingViDefectTracking_Model objModel )
		{
			Common.Model.MouldingViDefectTracking_Model model;
			model = new Common.Model.MouldingViDefectTracking_Model();
        ...
        #region MyRegion
        public MouldingViDefectTracking_Model GetModelList_ByDayShiftPartMachine_New(DateTime curDay, string curShift, string partNumber, string machineID)
        //public List<MouldingViDefectTracking_Model> GetModelList_ByDayShiftPartMachine_N(DateTime curDay, string curShift, string partNumber, string machineID)
        {

            List<MouldingViDefectTracking_Model> objVi = new List<MouldingViDefectTracking_Model>();
            return dal.GetModel_ByDayShiftPartMachine_New(curDay, curShift, partNumber, machineID);


            //DataSet ds = dal.GetModel_ByDayShiftPartMachine_New(curDay, curShift, partNumber, machineID);
        ...
        public List<MouldingViDefectTracking_Model> GetModelList_ByDayShiftPartMachine(DateTime curDay, string curShift, string partNumber, string machineID)
        {

            List<MouldingViDefectTracking_Model> objVi = new List<MouldingViDefectTracking_Model>();
            DataSet ds = dal.GetModel_ByDayShiftPartMachine(curDay, curShift, partNumber, machineID);
            if (ds == null || ds.Tables.Count == 0 || ds.Tables[0].Rows.Count == 0)
            {
                return null;
            }
        ...
        public MouldingViDefectTracking_Model GetModelList_ByDayShiftPartMachineForDefectTracking(DateTime? curDay, string curShift, string partNumber, string machineID,string defectcode,MouldingViDefectTracking_Model objViDef)
        {
            
            return dal.GetModel_ByDayShiftPartMachineForDefectTracking(curDay, curShift, partNumber, machineID, defectcode, objViDef);
        }

        public int getTotalRejQty_By_TrackingID(string trackingID)
        {
            DataSet ds = dal.GetList( " TrackingID = '"+ trackingID + "'");
            if(ds == null || ds.Tables.Count==0 || ds.Tables[0].Rows.Count ==0)
            {
                return 0;
            }
            ...
            {
                List<Common.Model.MouldingViDefectTracking_Model> objViDefList = new List<MouldingViDefectTracking_Model>();
                objViDefList = DataTableToList(ds.Tables[0]);
                decimal dTotalQty = 0;
                foreach (Common.Model.MouldingViDefectTracking_Model objTck in objViDefList)
                {
                    dTotalQty += objTck.rejectQty.Value;
                }
                return int.Parse(dTotalQty.ToString());
            }
        }
